...
public class SeriesIjkPlayActivity extends AppCompatActivity implements  SeekBar.OnSeekBarChangeListener, View.OnClickListener, IMediaPlayer.OnErrorListener, IMediaPlayer.OnCompletionListener{
    ...
    String cont_url,title,stream_id,img,name,rss="",type;
    int dration_time = 0,pos,position,msg_time = 0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        type = movieModels.get(pos).getType();
        ...
        new Thread(this::GetPlayUrl).start();
        FullScreencall();
    ...
    private void GetPlayUrl(){
        if(MyApp.is_mac){
            ...
            String  season_num = MyApp.selectedSeasonModel.getCategory();
            String series_id =MyApp.series_id;
            String str_cmd = "{\"type\":\"series\",\"series_id\":"+series_id+",\"season_num\":"+season_num+"}";
            Log.e("str_cmd",str_cmd);
            Log.e("episode_num",movieModels.get(pos).getNum()+"");
            cmd = Base64.encodeToString(str_cmd.getBytes(),Base64.DEFAULT).replaceAll("\\s+","").replaceAll("\n","");
            try {
                response = MyApp.instance.getIptvclient().macSeriesCmd(cmd,String.valueOf(Integer.parseInt(movieModels.get(pos).getNum())));
                Log.e("macVodCmd",response);
                jsonObject = new JSONObject(response);
                js = jsonObject.getJSONObject("js");
                cont_url= js.getString("cmd");
                cont_url = cont_url.replaceAll("ffmpeg","").replaceAll("auto","").replaceAll("\\s+","");
            }catch (Exception e){
                cont_url = MyApp.instance.getIptvclient().buildSeriesStreamURL(MyApp.user,MyApp.pass,stream_id,type);
            }
            if(cont_url==null || cont_url.isEmpty() || cont_url.equalsIgnoreCase("null")){
                cont_url = MyApp.instance.getIptvclient().buildSeriesStreamURL(MyApp.user,MyApp.pass,stream_id,type);
            }
        }else {
            cont_url = MyApp.instance.getIptvclient().buildSeriesStreamURL(MyApp.user,MyApp.pass,stream_id,type);
        ...
        runOnUiThread(()-> playVideo(cont_url));
    }
